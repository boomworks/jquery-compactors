...
*/
(function(d){d.fn.compactors=function(i){var b=d.extend({},d.fn.compactors.defaults,i),c=this.selector;return this.each(function(h){var f=d(this),g=f.parent(b.parent_container_selector);g.attr("role","tablist").attr("aria-multiselectable","true");f.addClass(b.enabled_class_name).bind({open:function(){e.stopPropagation();f.removeClass(b.closed_class_name).addClass(b.opened_class_name).find(b.content_selector).show(b.animation_speed).attr("aria-expanded","true")},close:function(){e.stopPropagation();
